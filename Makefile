...
DEFAULT_SOURCE_REPO ?= $(if $(wildcard fossology-gsoc/fossology/.git),fossology-gsoc/fossology,../fossology-gsoc/fossology)
FOSSOLOGY_REPO_DIR ?= $(DEFAULT_SOURCE_REPO)
BUILD_FOSSOLOGY_FROM_SOURCE ?= $(if $(wildcard $(FOSSOLOGY_REPO_DIR)/.git),1,0)
NAMESPACE ?= fossology
...
build:
	BUILD_FOSSOLOGY_FROM_SOURCE=$(BUILD_FOSSOLOGY_FROM_SOURCE) FOSSOLOGY_REPO_DIR=$(FOSSOLOGY_REPO_DIR) FOSSOLOGY_IMAGE=$(FOSSOLOGY_IMAGE) WORKER_IMAGE=$(WORKER_IMAGE) bash scripts/build-images.sh
...
up: build
	BUILD_FOSSOLOGY_FROM_SOURCE=$(BUILD_FOSSOLOGY_FROM_SOURCE) FOSSOLOGY_REPO_DIR=$(FOSSOLOGY_REPO_DIR) FOSSOLOGY_IMAGE=$(FOSSOLOGY_IMAGE) WORKER_IMAGE=$(WORKER_IMAGE) bash scripts/setup.sh
...
up-phase1: build
	BUILD_FOSSOLOGY_FROM_SOURCE=$(BUILD_FOSSOLOGY_FROM_SOURCE) DEPLOY_MODE=helm FOSSOLOGY_REPO_DIR=$(FOSSOLOGY_REPO_DIR) FOSSOLOGY_IMAGE=$(FOSSOLOGY_IMAGE) WORKER_IMAGE=$(WORKER_IMAGE) HELM_RELEASE=$(HELM_RELEASE) HELM_CHART=$(HELM_CHART) HELM_VALUES=$(HELM_VALUES) bash scripts/setup.sh
